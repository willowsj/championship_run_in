...
january  <- read.csv("january.csv",  stringsAsFactors = FALSE)
february <- read.csv("february.csv", stringsAsFactors = FALSE)
march    <- read.csv("march.csv",    stringsAsFactors = FALSE)
april    <- read.csv("april.csv",    stringsAsFactors = FALSE)
may      <- read.csv("may.csv",      stringsAsFactors = FALSE)
...
all_teams_list <- as.list(setNames(all_teams, all_teams))
...
ui <- fluidPage(
  ...
  titlePanel(tags$span(
    "EFL Championship 2025–26 Run-in",
  ...
  fluidRow(
    column(
      ...
      div(
        ...
        plotOutput("fixture_plot", width = "100%")
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  filtered_schedule <- reactive({
    ...
    bind_rows(
      all_fixtures %>% mutate(Team = Home, `Home/Away` = "Home", Opposition = Away),
      all_fixtures %>% mutate(Team = Away, `Home/Away` = "Away", Opposition = Home)
    ) %>%
      ...
      mutate(
        Fixture_PPG = if_else(`Home/Away` == "Home",
                              Opp_Away_PPG, Opp_Home_PPG),
  ...
  output$fixture_plot <- renderPlot({
    ...
    max_fixtures <- max(schedule %>% count(Team) %>% pull(n))
    
    text_size <- max(2.8, min(5.8, 600 / max_fixtures)) * 1
    ...
    plot_table_full <- lapply(selected_teams, function(tm) {
      ...
      tibble(
        Team = tm, Row = 0, Opp_Display = tm,
        Fixture_PPG = NA, Bold = TRUE
      ) %>%
        bind_rows(
          tibble(Team = tm, Row = 1:max_fixtures,
                 Opp_Display = c(team_sched$Opp_Display,
                                 rep("", max_fixtures - n_team)),
                 Fixture_PPG = c(team_sched$Fixture_PPG,
                                 rep(NA, max_fixtures - n_team)),
                 Bold = FALSE)
        ) %>%
        bind_rows(
          tibble(
    ...
    ggplot(plot_table_full, aes(y = -Row)) +
      ...
      geom_tile(
        data = plot_table_full %>% filter(!is.na(Fixture_PPG)),
        aes(x = 0.85, fill = Fixture_PPG),
        width = 0.1, height = 0.97
      ) +
      
      geom_text(
        aes(x = 0.05, label = Opp_Display,
      ...
      geom_text(
        aes(x = 0.85,
            label = ifelse(is.na(Fixture_PPG), "", Fixture_PPG),
            fontface = ifelse(Bold, "bold", "plain")),
        size = text_size, family = "roboto", color = "grey34"
      ) +
      
      # ✅ Separator below header
      geom_hline(yintercept = -0.5, color = "grey85", linewidth = 0.4) +
      
      # ✅ Separator above Fixture Difficulty
      geom_hline(yintercept = -(max_fixtures + 0.5),
                 color = "grey85", linewidth = 0.4) +
      
      scale_fill_gradient2(
        low = "#1B7837", mid = "white", high = "#B2182B",
        midpoint = 1.5, na.value = "transparent"
      ) +
